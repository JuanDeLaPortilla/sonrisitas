...
public class Secciones extends javax.swing.JFrame {
    ...
    public void listarSecciones() {
        ...
        modelo = (DefaultTableModel) tableSeccion.getModel();
        Object[] ob = new Object[4];
        for (int i = 0; i < listarSC.size(); i++) {
            ob[0] = listarSC.get(i).getSeccionID();
            ob[1] = listarSC.get(i).getNombre();
            ob[2] = listarSC.get(i).getProfesorID();
            ob[3] = seccion.buscarNombreProfesor((listarSC.get(i).getProfesorID()), (listarSC.get(i).getSeccionID()));
            modelo.addRow(ob);
        }
        tableSeccion.setModel(modelo);
    }
    ...
    public String generarIDProfesor(int id) {
    ...
    private void initComponents() {
        ...
        favicon.setText("Panel de Secciones");
        ...
        tableSeccion.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID SECCION", "NOMBRE ", "ID PROFESOR", "NOMBRE PROFESOR"
            }
        ));
    ...
    private void agregarBtnTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_agregarBtnTxtMouseClicked
        ...
        sc.setSeccionID(generarIDSeccion(Integer.parseInt(txtIdSeccion.getText())));
        sc.setNombre(txtNombre.getText());
        sc.setProfesorID(generarIDProfesor(Integer.parseInt(txtIdProfesor.getText())));
        seccion.RegistrarSeccion(sc);
        limpiarTabla();
        limpiarCampos();
        listarSecciones();
        JOptionPane.showMessageDialog(this, "Sección Registrada con Éxito ", "REGISTRO EXITOSO", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void buscarBtnTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_buscarBtnTxtMouseClicked
        if ("".equals(txtIdSeccion.getText()) || "Ingrese el ID".equals(txtIdSeccion.getText()) || null == (txtIdSeccion.getText())) {
            String id = JOptionPane.showInputDialog(null, "Ingrese el ID de la sección a buscar", "BUSCAR SECCIÓN", JOptionPane.INFORMATION_MESSAGE);
            if (id != null) {
                JOptionPane.showMessageDialog(null, (seccion.BuscarSeccion(id)), ("DATOS DE LA SECCIÓN " + id), JOptionPane.INFORMATION_MESSAGE);
                limpiarTabla();
                limpiarCampos();
                listarSecciones();
            }
        ...
        String id = txtIdSeccion.getText();
        JOptionPane.showMessageDialog(null, (seccion.BuscarSeccion(id)), ("DATOS DE LA SECCIÓN " + id), JOptionPane.INFORMATION_MESSAGE);
        limpiarTabla();
        limpiarCampos();
        listarSecciones();
    ...
    private void updateBtnTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_updateBtnTxtMouseClicked
        ...
        sc.setNombre(txtNombre.getText());
        sc.setProfesorID(generarIDProfesor(Integer.parseInt(txtIdProfesor.getText())));
        int confirmation = JOptionPane.showConfirmDialog(null, ("¿Estas seguro de actualizar la sección " + sc.getSeccionID() + " ?"), "ACTUALIZAR SECCIÓN", JOptionPane.YES_NO_OPTION);
        if (confirmation == 0) {
            seccion.ModificarSeccion(sc);
            limpiarTabla();
            limpiarCampos();
            listarSecciones();
            JOptionPane.showMessageDialog(this, "Sección Actualizada con Éxito ", "ACTUALIZACIÓN EXITOSA", JOptionPane.INFORMATION_MESSAGE);
        }
    ...
    private void deleteBtnTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_deleteBtnTxtMouseClicked
        if ("".equals(txtIdSeccion.getText()) || "Ingrese el ID".equals(txtIdSeccion.getText()) || null == txtIdSeccion) {
            String id = JOptionPane.showInputDialog(null, "Ingrese el ID de la sección a eliminar", "ELIMINAR SECCIÓN", JOptionPane.INFORMATION_MESSAGE);
            if (id != null) {
                int confirmation = JOptionPane.showConfirmDialog(null, ("¿Estas seguro de eliminar la sección " + id + " ?"), "ELIMINAR SECCIÓN", JOptionPane.YES_NO_OPTION);
                if (confirmation == 0) {
                    seccion.EliminarSeccion(id);
                    limpiarTabla();
                    limpiarCampos();
                    listarSecciones();
                    JOptionPane.showMessageDialog(this, "Sección Eliminada con Éxito ", "ELIMINACIÓN EXITOSA", JOptionPane.INFORMATION_MESSAGE);
                }
            }
            return;
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Secciones.class
